    ...
    try:
        while(True):
            ...
            for tn,te in dict.items( tags ):
                    tag_entries.append([tn,te.attribute.value])
                    # tag_entries.append(te.attribute.value)
            q.put((tag_entries,ts))
            # print("Created a queue entry")
            time.sleep(10)
    finally:
        print("Stopping...")
...
def snapshot_consumer(q,filename):
    ...
    while True:
        ...
        for elem in snapshot:
            tagname = elem[0].split(':2')[0]
            tagval = elem[1]
            if tagname in tags_to_log:
                tagvalues[tags_to_log.index(tagname)] = tagval
        ...
        # fd = os.open(filename, os.O_WRONLY | os.O_CREAT | os.O_APPEND | os.O_SYNC)
...
#
def main( argv=None, attribute_class=device.Attribute, idle_service=None, identity_class=None, UCMM_class=None, message_router_class=None, connection_manager_class=None, **kwds ):
    ...
    # q = mp.Queue()
    # consumer = mp.Process(target=snapshot_consumer, args=(q,'/home/ubuntu/test-setup/scadalogs/logs.csv',))
