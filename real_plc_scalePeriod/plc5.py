...
class plc5(PLC):
	...
	def fetchData(self):
		# print("PLC5 data transmission starting at", time.time())
		# if state==1:
	...
	def Iteration(self,timer):
		p = multiprocessing.Process(target=self.fetchData)
		p.start()
		p.join(timeout)
		if p.is_alive():
...
# 		self.hmimv501status_upd, self.hmimv501avl = self.MV501_FB.MV_FBD(self.Mid_MV501_AutoInp, self.IO.MV501, self.hmimv501auto)
...
# 		self.hmimv502status_upd, self.hmimv502avl = self.MV502_FB.MV_FBD(self.Mid_MV502_AutoInp, self.IO.MV502, self.hmimv502auto)
...
# 		self.hmimv503status_upd, self.hmimv503avl = self.MV503_FB.MV_FBD(self.Mid_MV503_AutoInp, self.IO.MV503, self.hmimv503auto)
...
# 		self.hmimv504status_upd, self.hmimv504avl = self.MV504_FB.MV_FBD(self.Mid_MV504_AutoInp, self.IO.MV504, self.hmimv504auto)
...
# 		setdata(self, 'HMI.P_RO_HIGH_DUTY.Both_Pmp_Not_Avl',SCADA_ADDR,self.hmiprohighdutybothpmpnotavl)
		...
		print("PLC5 data transmission ending at", time.time())
		elapsed = time.time() - iter_start
		print('PLC5 Iteration time:', elapsed)
