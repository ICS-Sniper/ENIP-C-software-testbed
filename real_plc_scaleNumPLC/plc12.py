...
class plc12(PLC):
	...
	def fetchData(self,state):
		print("PLC12 data transmission starting at", time.time())
		if state == 1:
			self.shared["hmiplantstart"]= getdata(self, 'HMI.PLANT.Start',SCADA_ADDR,self.shared["hmiplantstart"])
		if state == 2:
			self.shared["hmiait202pv"] =getdata(self, 'HMI.AIT802.Pv',SCADA_ADDR,self.shared["hmiait202pv"])
			self.shared["hmilit101ahh"] = getdata(self, 'HMI.LIT701.AHH',SCADA_ADDR,self.shared["hmilit101ahh"])
			self.shared["hmimidp602autoinp"] = getdata(self, 'HMI.Mid_P602_AutoInp_DUP',SCADA_ADDR,self.shared["hmimidp602autoinp"])
		...
		self.shared["hmimv301open"] = getdata(self, 'HMI.MV901.Open',SCADA_ADDR,self.shared["hmimv301open"])
		...
		self.shared["hmimv303open"] = getdata(self, 'HMI.MV903.Open',SCADA_ADDR,self.shared["hmimv303open"])
		...
		self.shared["hmip401status"] = getdata(self, 'HMI.P1001.Status',SCADA_ADDR,self.shared["hmip401status"])
		self.shared["hmip402status"] = getdata(self, 'HMI.P1002.Status',SCADA_ADDR,self.shared["hmip402status"])
		self.shared["hmip501status"] = getdata(self, 'HMI.P1101.Status',SCADA_ADDR,self.shared["hmip501status"])
		self.shared["hmip502status"] = getdata(self, 'HMI.P1102.Status',SCADA_ADDR,self.shared["hmip502status"])
		self.shared["hmimv501open"] = getdata(self, 'HMI.MV1101.Open',SCADA_ADDR,self.shared["hmimv501open"])
		self.shared["hmimv501close"] = getdata(self, 'HMI.MV1101.Close',SCADA_ADDR,self.shared["hmimv501close"])
		self.shared["hmimv502open"] = getdata(self, 'HMI.MV1102.Open',SCADA_ADDR,self.shared["hmimv502open"])
		self.shared["hmimv502close"] = getdata(self, 'HMI.MV1102.Close',SCADA_ADDR,self.shared["hmimv502close"])
		self.shared["hmimv503open"] = getdata(self, 'HMI.MV1103.Open',SCADA_ADDR,self.shared["hmimv503open"])
		self.shared["hmimv503close"] = getdata(self, 'HMI.MV1103.Close',SCADA_ADDR,self.shared["hmimv503close"])
		self.shared["hmimv504open"] = getdata(self, 'HMI.MV1104.Open',SCADA_ADDR,self.shared["hmimv504open"])
		self.shared["hmimv504close"] = getdata(self, 'HMI.MV1104.Close',SCADA_ADDR,self.shared["hmimv504close"])
	...
	def Actuator(self):
		# pass
		self.IO.P1201.DI_Run = self.IO.P1201.DO_Start
		self.IO.P1202.DI_Run = self.IO.P1202.DO_Start
	...
	def Plant(self):
		...
		if self.hmip301status == 1 and self.hmip302status == 1 and self.hmimv301open==1 and  self.hmimv302close == 1 and self.hmimv303open == 1 and self.hmimv304close == 1 and self.IO.P1202.DI_Run == 1:   #UF back wash procedure, 45 sec
			self.h_t602=self.h_t602- self.p['f_p602'] / self.p['S_t602']

		if self.hmip301status == 1 and self.hmip302status == 1 and self.hmimv301close == 1 and  self.hmimv302close == 1 and self.hmimv303close == 1 and self.hmimv304open == 1 and self.IO.P1202.DI_Run == 0:   #UF feed tank draining procedure, 1 min
			pass
		...
		if self.IO.P1201.DI_Run == 1: # Pumping water out of tank601
	...
	def Iteration(self):
		...
		print("PLC12 State:", self.hmip6state)
...
#Product
		# self.hmilsl601alarm=
		self.LSL601_FB.SWITCH_FBD(self.IO.LSL1201)
		# self.hmilsl602alarm=
		self.LSL602_FB.SWITCH_FBD(self.IO.LSL1202)
		# self.hmilsl603alarm=
		self.LSL603_FB.SWITCH_FBD(self.IO.LSL1203)
		# self.hmilsh601alarm=
		self.LSH601_FB.SWITCH_FBD(self.IO.LSH1201)
		# self.hmilsh602alarm=
		self.LSH602_FB.SWITCH_FBD(self.IO.LSH1202)
		# self.hmilsh603alarm=
		self.LSH603_FB.SWITCH_FBD(self.IO.LSH1203)

		# self.P601.PMP_FBD(self.Mid_P601_AutoInp, self.IO.P601,HMI.P601)
		self.hmip601status_upd, self.hmip601fault, self.hmip601avl, self.hmip601shutdown = self.P601.PMP_FBD(self.Mid_P601_AutoInp, self.IO.P1201, self.hmip601auto, self.hmip601reset, self.hmip601permissive, self.hmip601sd)
		...
		self.hmip602status_upd, self.hmip602fault, self.hmip602avl, self.hmip602shutdown = self.P602.PMP_FBD(self.Mid_P602_AutoInp, self.IO.P1202, self.hmip602auto, self.hmip602reset, self.hmip602permissive, self.hmip602sd)
		...
		self.hmip603status_upd, self.hmip603fault, self.hmip603avl, self.hmip603shutdown = self.P603.PMP_FBD(self.Mid_P603_AutoInp, self.IO.P1203, self.hmip603auto, self.hmip603reset, self.hmip603permissive, self.hmip603sd)
		...
		setdata(self, 'HMI.P1201.Reset',SCADA_ADDR,self.hmip601reset)
		setdata(self, 'HMI.P1202.Reset',SCADA_ADDR,self.hmip602reset)
		setdata(self, 'HMI.P1203.Reset',SCADA_ADDR,self.hmip603reset)
		setdata(self, 'HMI.P1201.Auto',SCADA_ADDR,self.hmip601auto)
		setdata(self, 'HMI.P1202.Auto',SCADA_ADDR,self.hmip602auto)
		setdata(self, 'HMI.P1203.Auto',SCADA_ADDR,self.hmip603auto)
		setdata(self, 'HMI.P12.Permissive_On',SCADA_ADDR,self.hmip6permissiveon)
		setdata(self, 'HMI.P1201.Permissive',SCADA_ADDR,self.hmip601permissive)
		setdata(self, 'HMI.P1201.MSG_Permissive',SCADA_ADDR,self.hmip601msgpermissive)
		setdata(self, 'HMI.P1201.SD',SCADA_ADDR,self.hmip601sd)
		setdata(self, 'HMI.P1201.MSG_Shutdown',SCADA_ADDR,self.hmip601msgshutdown)
		setdata(self, 'HMI.P1202.Permissive',SCADA_ADDR,self.hmip602permissive)
		setdata(self, 'HMI.P1202.MSG_Permissive',SCADA_ADDR,self.hmip602msgpermissive)
		setdata(self, 'HMI.P1202.SD',SCADA_ADDR,self.hmip602sd)
		setdata(self, 'HMI.P1202.MSG_Shutdown',SCADA_ADDR,self.hmip602msgshutdown)
		setdata(self, 'HMI.P1203.Permissive',SCADA_ADDR,self.hmip603permissive)
		setdata(self, 'HMI.P1203.MSG_Permissive',SCADA_ADDR,self.hmip603msgpermissive)
		setdata(self, 'HMI.P1203.SD',SCADA_ADDR,self.hmip603sd)
		setdata(self, 'HMI.P1203.MSG_Shutdown',SCADA_ADDR,self.hmip603msgshutdown)
		if self.hmiplantstop or self.hmiplantcriticalsdon:
		    setdata(self, 'HMI.P12.State',SCADA_ADDR,1)

		if self.hmip6state_prev == 1:
		    if self.hmip6state == 2:
		        setdata(self, 'HMI.P12.State',SCADA_ADDR,2)
		        self.hmip6state_prev= 0
		if self.hmip6state_prev > 2:
		    setdata(self, 'HMI.P12.State',SCADA_ADDR,1)
		    self.hmip6state_prev = 0

		setdata(self, 'HMI.LSL1201.Alarm',SCADA_ADDR,self.hmilsl601alarm)
		setdata(self, 'HMI.LSL1202.Alarm',SCADA_ADDR,self.hmilsl602alarm)
		setdata(self, 'HMI.LSL1203.Alarm',SCADA_ADDR,self.hmilsl603alarm)
		setdata(self, 'HMI.LSH1201.Alarm',SCADA_ADDR,self.hmilsh601alarm)
		setdata(self, 'HMI.LSH1202.Alarm',SCADA_ADDR,self.hmilsh602alarm)
		setdata(self, 'HMI.LSH1203.Alarm',SCADA_ADDR,self.hmilsh603alarm)
		setdata(self, 'HMI.P1201.Status',SCADA_ADDR,self.hmip601status)
		setdata(self, 'HMI.P1201.Fault',SCADA_ADDR,self.hmip601fault)
		setdata(self, 'HMI.P1201.Avl',SCADA_ADDR,self.hmip601avl)
		setdata(self, 'HMI.P1201.Shutdown',SCADA_ADDR,self.hmip601shutdown)
		setdata(self, 'HMI.P1202.Status',SCADA_ADDR,self.hmip602status)
		setdata(self, 'HMI.P1202.Fault',SCADA_ADDR,self.hmip602fault)
		setdata(self, 'HMI.P1202.Avl',SCADA_ADDR,self.hmip602avl)
		setdata(self, 'HMI.P1202.Shutdown',SCADA_ADDR,self.hmip602shutdown)
		setdata(self, 'HMI.P1203.Status',SCADA_ADDR,self.hmip603status)
		setdata(self, 'HMI.P1203.Fault',SCADA_ADDR,self.hmip603fault)
		setdata(self, 'HMI.P1203.Avl',SCADA_ADDR,self.hmip603avl)
		setdata(self, 'HMI.P1203.Shutdown',SCADA_ADDR,self.hmip603shutdown)
		setdata(self, 'HMI.LSH1201.Alarm',SCADA_ADDR,self.hmilsh601alarm)
		setdata(self, 'HMI.LSL1201.Alarm',SCADA_ADDR,self.hmilsl601alarm)
		setdata(self, 'HMI.LSH1202.Alarm',SCADA_ADDR,self.hmilsh602alarm)
		setdata(self, 'HMI.LSL1202.Alarm',SCADA_ADDR,self.hmilsl602alarm)
		print("PLC12 data transmission ending at", time.time())
		#### setdata() calls end here ########
		elapsed = time.time() - iter_start
		print('PLC12 Iteration time:', elapsed)
