...
class plc10(PLC):
	...
	def fetchData(self, state):
		print("PLC10 data transmission starting at", time.time())
		self.shared["hmimv501status"] = getdata(self, 'HMI.MV1101.Status',SCADA_ADDR,self.shared["hmimv501status"])
		self.shared["hmimv502status"] = getdata(self, 'HMI.MV1102.Status',SCADA_ADDR,self.shared["hmimv502status"])
		self.shared["hmimv503status"] = getdata(self, 'HMI.MV1103.Status',SCADA_ADDR,self.shared["hmimv503status"])
		self.shared["hmimv504status"] = getdata(self, 'HMI.MV1104.Status',SCADA_ADDR,self.shared["hmimv504status"])
		...
		self.shared["hmip301status"] = getdata(self, 'HMI.P901.Status',SCADA_ADDR,self.shared["hmip301status"])
		self.shared["hmip302status"] = getdata(self, 'HMI.P902.Status',SCADA_ADDR,self.shared["hmip302status"])
		self.shared["hmimv301close"] = getdata(self, 'HMI.MV901.Close',SCADA_ADDR,self.shared["hmimv301close"])
		self.shared["hmimv302close"] = getdata(self, 'HMI.MV902.Close',SCADA_ADDR,self.shared["hmimv302close"])
		self.shared["hmimv303close"] = getdata(self, 'HMI.MV903.Close',SCADA_ADDR,self.shared["hmimv303close"])
		self.shared["hmimv304open"] = getdata(self, 'HMI.MV904.Open',SCADA_ADDR,self.shared["hmimv304open"])
		self.shared["hmip602status"] = getdata(self, 'HMI.P1202.Status',SCADA_ADDR,self.shared["hmip602status"])
		self.shared["hmimv302open"] = getdata(self, 'HMI.MV902.Open',SCADA_ADDR,self.shared["hmimv302open"])
		self.shared["hmimv304close"] = getdata(self, 'HMI.MV904.Close',SCADA_ADDR,self.shared["hmimv304close"])
		if state == 1:
			self.shared["hmiplantstart"] = getdata(self, 'HMI.PLANT.Start',SCADA_ADDR,self.shared["hmiplantstart"])


	def Actuator(self):
		self.IO.P1001.DI_Run = self.IO.P1001.DO_Start
		self.IO.P1002.DI_Run = self.IO.P1002.DO_Start
	...
	def Plant(self):
		...
		if self.IO.P1001.DI_Run == 1 or self.IO.P1002.DI_Run == 1: #P401, drawing water from t401
	...
	def Iteration(self):
		...
		print("PLC10 State:", self.hmip4state)
...
# Main
		if self.hmip4state == 1:
			...
			self.Mid_P_NAHSO3_ORP_DUTY_AutoInp	=0
			print("PLC10 Debug:self.hmip4state, self.hmip4permissiveon, self.hmiplantstart", self.hmip4state, self.hmip4permissiveon, self.hmiplantstart)
...
# RO_Feed_Dosing
		...
		self.hmip401status_upd, self.hmip401fault, self.hmip401avl, self.hmip401shutdown = self.P401_FB.PMP_FBD(self.P_RO_FEED_DUTY_FB.Start_Pmp1, self.IO.P1001, self.hmip401auto, self.hmip401reset, self.hmip401permissive, self.hmip401sd)
		...
		self.hmip402status_upd, self.hmip402fault, self.hmip402avl, self.hmip402shutdown = self.P402_FB.PMP_FBD(self.P_RO_FEED_DUTY_FB.Start_Pmp2, self.IO.P1002, self.hmip402auto, self.hmip402reset, self.hmip402permissive, self.hmip402sd)
		...
		self.hmiuv401status_upd, self.hmiuv401fault_upd, self.hmiuv401avl_upd, self.hmiuv401runhr_upd, self.hmiuv401totalrunhr_upd, self.hmiuv401shutdown_upd = self.UV401_FB.UV_FBD(self.Mid_UV401_AutoInp, self.IO.UV1001, self.hmiuv401auto, self.hmiuv401reset, self.hmiuv401resetrunhr, self.hmiuv401permissive, self.hmiuv401sd)
		...
		# self.hmils401alarm =
		self.LS401_FB.SWITCH_FBD(self.IO.LS1001)
		...
		self.hmip403status_upd, self.hmip403fault, self.hmip403avl, self.hmip403shutdown = self.P403_FB.PMP_FBD(self.P_NAHSO3_ORP_DUTY_FB.Start_Pmp1, self.IO.P1003, self.hmip403auto, self.hmip403reset, self.hmip403permissive, self.hmip403sd)
		...
		self.hmip404status_upd, self.hmip404fault, self.hmip404avl, self.hmip404shutdown = self.P404_FB.PMP_FBD(self.P_NAHSO3_ORP_DUTY_FB.Start_Pmp2, self.IO.P1004, self.hmip404auto, self.hmip404reset, self.hmip404permissive, self.hmip404sd)
		...
		setdata(self, 'HMI.P1001.Reset',SCADA_ADDR,self.hmip401reset)
		setdata(self, 'HMI.P1002.Reset',SCADA_ADDR,self.hmip402reset)
		setdata(self, 'HMI.P1003.Reset',SCADA_ADDR,self.hmip403reset)
		setdata(self, 'HMI.P1004.Reset',SCADA_ADDR,self.hmip404reset)
		setdata(self, 'HMI.UV1001.Reset',SCADA_ADDR,self.hmiuv401reset)
		setdata(self, 'HMI.P1001.Auto',SCADA_ADDR,self.hmip401auto)
		setdata(self, 'HMI.P1002.Auto',SCADA_ADDR,self.hmip402auto)
		setdata(self, 'HMI.P1003.Auto',SCADA_ADDR,self.hmip403auto)
		setdata(self, 'HMI.P1004.Auto',SCADA_ADDR,self.hmip404auto)
		setdata(self, 'HMI.UV1001.Auto',SCADA_ADDR,self.hmiuv401auto)
		setdata(self, 'HMI.P10.Permissive_On',SCADA_ADDR,self.hmip4permissiveon)
		setdata(self, 'HMI.P1001.Permissive',SCADA_ADDR,self.hmip401permissive)
		setdata(self, 'HMI.P1001.MSG_Permissive',SCADA_ADDR,self.hmip401msgpermissive)
		setdata(self, 'HMI.P1002.Permissive',SCADA_ADDR,self.hmip402permissive)
		setdata(self, 'HMI.P1002.MSG_Permissive',SCADA_ADDR,self.hmip402msgpermissive)
		setdata(self, 'HMI.P1003.Permissive',SCADA_ADDR,self.hmip403permissive)
		setdata(self, 'HMI.P1003.MSG_Permissive',SCADA_ADDR,self.hmip403msgpermissive)
		setdata(self, 'HMI.P1004.Permissive',SCADA_ADDR,self.hmip404permissive)
		setdata(self, 'HMI.P1004.MSG_Permissive',SCADA_ADDR,self.hmip404msgpermissive)
		setdata(self, 'HMI.UV1001.Permissive',SCADA_ADDR,self.hmiuv401permissive)
		setdata(self, 'HMI.UV1001.MSG_Permissive',SCADA_ADDR,self.hmiuv401msgpermissive)
		setdata(self, 'HMI.P1001.SD',SCADA_ADDR,self.hmip401sd)
		setdata(self, 'HMI.P1001.MSG_Shutdown',SCADA_ADDR,self.hmip401msgshutdown)
		setdata(self, 'HMI.P1002.SD',SCADA_ADDR,self.hmip402sd)
		setdata(self, 'HMI.P1002.MSG_Shutdown',SCADA_ADDR,self.hmip402msgshutdown)
		setdata(self, 'HMI.P1003.SD',SCADA_ADDR,self.hmip403sd)
		setdata(self, 'HMI.P1003.MSG_Shutdown',SCADA_ADDR,self.hmip403msgshutdown)
		setdata(self, 'HMI.P1004.SD',SCADA_ADDR,self.hmip404sd)
		setdata(self, 'HMI.P1004.MSG_Shutdown',SCADA_ADDR,self.hmip404msgshutdown)
		setdata(self, 'HMI.UV1001.SD',SCADA_ADDR,self.hmiuv401sd)
		setdata(self, 'HMI.UV1001.MSG_Shutdown',SCADA_ADDR,self.hmiuv401msgshutdown)
		if self.hmip4state_prev == 1:
		  if self.hmip4state == 2:
		    setdata(self, 'HMI.P10.State',SCADA_ADDR,2)
		    self.hmip4state_prev = 0
		if self.hmip4state_prev == 2:
		  if self.hmip4state == 3:
		    setdata(self, 'HMI.P10.State',SCADA_ADDR,3)
		    self.hmip4state_prev = 0
		if self.hmip4state_prev == 3:
		  if self.hmip4state == 4:
		    setdata(self, 'HMI.P10.State',SCADA_ADDR,4)
		    self.hmip4state_prev = 0
		if self.hmip4state_prev == 4:
		  if self.hmip4state == 5:
		    setdata(self, 'HMI.P10.State',SCADA_ADDR,5)
		    self.hmip4state_prev = 0
		if self.hmip4state_prev == 5:
		  if self.hmip4state == 6:
		    setdata(self, 'HMI.P10.State',SCADA_ADDR,6)
		    self.hmip4state_prev = 0
		if self.hmip4state_prev == 6:
		  if self.hmip4state == 1:
		    setdata(self, 'HMI.P10.State',SCADA_ADDR,1)
		    self.hmip4state_prev = 0
		if self.hmip4state_prev > 6:
		  setdata(self, 'HMI.P10.State',SCADA_ADDR,1)

		setdata(self, 'HMI.LIT1001.Pv',SCADA_ADDR,self.hmilit401pv)
		setdata(self, 'HMI.LIT1001.AHH',SCADA_ADDR,self.hmilit401ahh)
		setdata(self, 'HMI.LIT1001.AH',SCADA_ADDR,self.hmilit401ah)
		setdata(self, 'HMI.LIT1001.AL',SCADA_ADDR,self.hmilit401al)
		setdata(self, 'HMI.LIT1001.ALL',SCADA_ADDR,self.hmilit401all)


		setdata(self, 'HMI.P_RO_FEED_DUTY_DUP.Both_Pmp_Not_Avl',SCADA_ADDR,self.hmiprofeeddutybothpmpnotavl)
		setdata(self, 'HMI.P_RO_FEED_DUTY_DUP.Selected_Pmp_Not_Avl',SCADA_ADDR,self.hmiprofeeddutyselectedpmpnotavl)
		setdata(self, 'HMI.P_RO_FEED_DUTY_DUP.Pump_Running',SCADA_ADDR,self.hmiprofeeddutypumprunning)
		setdata(self, 'HMI.P1001.Status',SCADA_ADDR,self.hmip401status)
		setdata(self, 'HMI.P1001.Fault',SCADA_ADDR,self.hmip401fault)
		setdata(self, 'HMI.P1001.Avl',SCADA_ADDR,self.hmip401avl)
		setdata(self, 'HMI.P1001.Shutdown',SCADA_ADDR,self.hmip401shutdown)
		setdata(self, 'HMI.P1002.Status',SCADA_ADDR,self.hmip402status)
		setdata(self, 'HMI.P1002.Fault',SCADA_ADDR,self.hmip402fault)
		setdata(self, 'HMI.P1002.Avl',SCADA_ADDR,self.hmip402avl)
		setdata(self, 'HMI.P1002.Shutdown',SCADA_ADDR,self.hmip402shutdown)
		setdata(self, 'HMI.UV1001.Status',SCADA_ADDR,self.hmiuv401status)
		setdata(self, 'HMI.UV1001.Avl',SCADA_ADDR,self.hmiuv401avl)
		setdata(self, 'HMI.UV1001.RunHr',SCADA_ADDR,self.hmiuv401runhr)
		setdata(self, 'HMI.UV1001.Shutdown',SCADA_ADDR,self.hmiuv401shutdown)
		setdata(self, 'HMI.LS1001.Alarm',SCADA_ADDR,self.hmils401alarm)
		setdata(self, 'HMI.P_NAHSO3_ORP_DUTY_DUP.Both_Pmp_Not_Avl',SCADA_ADDR,self.hmipnahso3orpdutybothpmpnotavl)
		setdata(self, 'HMI.P_NAHSO3_ORP_DUTY_DUP.Selected_Pmp_Not_Avl',SCADA_ADDR,self.hmipnahso3orpdutyselectedpmpnotavl)
		setdata(self, 'HMI.P_NAHSO3_ORP_DUTY_DUP.Pump_Running',SCADA_ADDR,self.hmipnahso3orpdutypumprunning)
		setdata(self, 'HMI.P1003.Status',SCADA_ADDR,self.hmip403status)
		setdata(self, 'HMI.P1003.Fault',SCADA_ADDR,self.hmip403fault)
		setdata(self, 'HMI.P1003.Avl',SCADA_ADDR,self.hmip403avl)
		setdata(self, 'HMI.P1003.Shutdown',SCADA_ADDR,self.hmip403shutdown)
		setdata(self, 'HMI.P1004.Status',SCADA_ADDR,self.hmip404status)
		setdata(self, 'HMI.P1004.Fault',SCADA_ADDR,self.hmip404fault)
		setdata(self, 'HMI.P1004.Avl',SCADA_ADDR,self.hmip404avl)
		setdata(self, 'HMI.P1004.Shutdown',SCADA_ADDR,self.hmip404shutdown)
		print("PLC10 data transmission ending at", time.time())
		# print("PLC10 Debug: HMI.LIT401.Pv, HMI.P301.Status, HMI.P302.Status, HMI.MV301.Close, HMI.MV302.Open, HMI.MV303.Close, HMI.MV304.Close, HMI.P602.Status, IO_P4.P401.DI_Run, IO_P4.P402.DI_Run", self.hmilit401pv, self.hmip301status, HMI.P302.Status, HMI.MV301.Close, HMI.MV302.Open, HMI.MV303.Close, HMI.MV304.Close, HMI.P602.Status, IO_P4.P401.DI_Run, IO_P4.P402.DI_Run)
		## setdata() calls end here ###################
		elapsed = time.time() - iter_start
		print('PLC10 Iteration time:', elapsed)
